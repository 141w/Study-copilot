...
1. **Batch 1（本周）**：#1–#4（本分支 `feat/agent-p0-hardening`）。
2. **Batch 2（2–6 周）**：#5 → #7 → #6 → #9 → 其余。
3. **Batch 3（持续）**：#12 评测集尽早启动；#13 深挖 Langfuse；#14/#15 按运维成熟度推进。

---

## 本轮不在范围

- 不引入 OTel / KMS / SSE Resume 实现
- 不修改纠检重试次数（已正确硬限 1）
- 不重构洋葱管线插件拓扑
