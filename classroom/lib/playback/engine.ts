        ...
        this.audioPlayer
          ...
          .then(async (audioStarted) => {
            ...
            if (!audioStarted) {
              ...
              if (!hasText || settings.ttsMuted) {
                scheduleReadingTimer();
                return;
              }

              // Dynamic TTS fallback: attempt to synthesize via backend TTS
              try {
                const speed = this.callbacks.getPlaybackSpeed?.() ?? 1;
                const ttsEndpoint =
                ...
                if (resp.ok) {
                  ...
                  if (blob && blob.size > 0 && this.isCurrentGeneration(generation)) {
                    const dynamicUrl = URL.createObjectURL(blob);
                    const played = await this.audioPlayer.play('', dynamicUrl);
                    if (played) return;
                  }
                }
              } catch (e) {
                log.warn('[PlaybackEngine] Dynamic TTS fetch failed, trying browser TTS', e);
              }

              if (!this.isCurrentGeneration(generation)) return;

              // Fallback to browser-native TTS (Web Speech API)
