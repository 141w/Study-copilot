...
export interface GenerateClassroomInput {
  ...
  pdfContent?:
    ...
    | Array<{ text: string; images?: string[]; filename?: string }>;
...
export async function generateClassroom(
  ...
  if (Array.isArray(pdfContent)) {
    const parts = (pdfContent as Array<{ text?: string; filename?: string }>)
      .map((d) => {
        const header = d.filename ? `【参考材料：${d.filename}】\n` : '';
        return `${header}${d.text || ''}`.trim();
      })
      .filter(Boolean);
    pdfText = parts.length > 0 ? parts.join('\n\n---\n\n') : undefined;
  } else if (pdfContent && typeof pdfContent === 'object') {
    pdfText = (pdfContent as { text?: string }).text || undefined;
  }
