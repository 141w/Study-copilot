...
function buildInputSummary(input: GenerateClassroomInput): ClassroomGenerationJob['inputSummary'] {
  ...
  const pdfContent = input.pdfContent as any;
  if (Array.isArray(pdfContent)) {
    hasPdf = pdfContent.length > 0;
    pdfTextLength = pdfContent.reduce((acc: number, doc: any) => acc + (doc?.text?.length || 0), 0);
    pdfImageCount = pdfContent.reduce((acc: number, doc: any) => acc + (doc?.images?.length || 0), 0);
  } else if (pdfContent && typeof pdfContent === 'object') {
    hasPdf = !!pdfContent.text;
    pdfTextLength = pdfContent.text?.length || 0;
    pdfImageCount = Array.isArray(pdfContent.images) ? pdfContent.images.length : 0;
  }
