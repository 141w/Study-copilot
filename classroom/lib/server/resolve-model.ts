...
export async function resolveModel(params: {
  ...
  let modelString = stageModel || params.modelString || process.env.DEFAULT_MODEL;
  if (
    !modelString ||
    modelString === ':' ||
    modelString.endsWith(':') ||
    !modelString.split(':')[1]?.trim()
  ) {
    modelString = stageModel || process.env.DEFAULT_MODEL || modelString;
  }
  if (!modelString) {
    throw new Error(
  ...
  if (!modelId && process.env.DEFAULT_MODEL) {
    const fallback = parseModelString(process.env.DEFAULT_MODEL);
    if (fallback.modelId) {
