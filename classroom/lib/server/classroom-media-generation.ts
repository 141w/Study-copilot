    ...
    for (const req of imageRequests) {
      try {
        ...
        const enhancedPrompt =
          rawPrompt.length > 120 || rawPrompt.toLowerCase().includes('detailed')
            ? rawPrompt
            : `High-quality educational concept illustration, clean infographic visual, professional diagram, modern pedagogy, 4k resolution, highly detailed: ${rawPrompt}`;

        const result = await generateImage(
          { providerId, apiKey, baseUrl: resolveImageBaseUrl(providerId), model },
          resolveImageSize(
            { prompt: enhancedPrompt, aspectRatio: req.aspectRatio || '16:9' },
