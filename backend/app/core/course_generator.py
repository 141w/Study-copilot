...
async def _load_document_context(
    db: AsyncSession, doc_ids: list[str], max_chars: int = 8000
) -> str:
    """从多个文档加载文本上下文。"""
    if not doc_ids:
        return ""
...
OUTLINE_PROMPT = """你是一位课程设计专家。请根据提供的文档内容，生成一份结构化的课程大纲。
...
async def generate_outline(
    ...
    prompt = OUTLINE_PROMPT.format(context=context[:6000])
...
async def generate_course(
    ...
    """
    # 1. 加载文档上下文
    context = await _load_document_context(db, doc_ids, max_chars=8000)
    if not context:
    ...
    for section in outline.get("sections", [])[:5]:  # 最多 5 个章节
        section_context = f"{section['title']}: {', '.join(section.get('key_points', []))}\n\n{context[:3000]}"
