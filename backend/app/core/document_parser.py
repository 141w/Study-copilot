...
改进 (2025-05-13):
- PDF 解析恢复真实页码：利用 PyMuPDF 按页提取文本，
  而非将整个文档合并为 1 页
...
class PDFParser(BaseParser):
    ...
    改进 (2025-05-13):
    - extract_pages 不再合并为 1 页，而是利用 PyMuPDF 按页提取，
      保证下游 chunker 能拿到真实的 page 编号。
    """
    ...
    def _convert_with_docling(self, file_path: str, enable_ocr: bool = False) -> dict[str, Any]:
        ...
        try:
            ...
            return {"metadata": metadata, "markdown": markdown_text, "success": True}
    ...
    def _split_markdown_pages(self, markdown_text: str, page_count: int) -> list[dict[str, Any]]:
        """
        将 Markdown 文本分割成页面块（改进版）。

        旧版直接将整个 Markdown 作为 1 页返回，导致下游 chunker 丢失页码。
        新版策略：
        1. 按 Markdown 标题层级拆分（保持语义）
        2. 如果文本很长（>50KB），按标题拆分成多页
        3. 否则仍返回单页（兼容旧逻辑）
        """
        if not markdown_text or not markdown_text.strip():
            return [{"page": 1, "text": "", "images": []}]

        # 策略1: 文本较短，直接返回整页（保持 Markdown 结构完整）
        if len(markdown_text) <= 30000:
            return [{"page": 1, "text": markdown_text, "images": []}]
        ...
        for idx, section in enumerate(sections):
            if not section.strip():
                continue
            # section 可能是标题标记（## 或 ###）或正文
            if section in ("##", "###"):
                # 下一个元素是标题文字
                continue
            if idx > 0 and sections[idx - 1] in ("##", "###"):
                section = f"{sections[idx - 1]} {section}"

            current_text.append(section.strip())

            # 每积累一定长度或遇到二级标题时分页
            accumulated = "\n\n".join(current_text)
            if len(accumulated) > 8000 or section.startswith("## "):
                pages.append({"page": current_page, "text": accumulated, "images": []})
                current_page += 1
                current_text = []
        ...
        try:
            ...
            if has_text:
                ...
                if page_count > self.LARGE_DOC_THRESHOLD:
                    # 大文档直接用 PyMuPDF
                    logger.info("Large doc with text, using PyMuPDF")
                    result = await loop.run_in_executor(
                        _executor, self._convert_fallback, file_path
                    )
                else:
                    # 小文档用 Docling 获得更好解析
                    logger.info("Small doc with text, using Docling")
                    result = await loop.run_in_executor(
                        _executor, self._convert_with_docling, file_path, False
                    )
            ...
            if not markdown_text or len(markdown_text.strip()) < 50:
                raise ValueError("无法提取有效文本内容。如果这是扫描件，请先转换为可搜索 PDF。")
    ...
    async def extract_pages(self, file_path: str) -> list[dict[str, Any]]:
        ...
        旧版：直接返回 parse() 的 pages，PDF 时通常只有 1 页。
        新版：如果 PDF 只有 1 页但页数很多，利用 PyMuPDF 按真实页码重新分页。
        """
        result = await self.parse(file_path)
        pages = result.get("pages", [])

        # 如果只有 1 页但 metadata 显示多页，尝试用 PyMuPDF 按页提取
        ...
        if len(pages) == 1 and page_count > 1:
            try:
                ...
                logger.warning(f"PyMuPDF repagination failed: {e}")
...
class DOCXParser(BaseParser):
    ...
    async def parse(self, file_path: str) -> dict[str, Any]:
        ...
        for table in doc.tables:
            ...
            if table_text:
                text_content.append({"page": page_num, "text": "\n".join(table_text), "images": []})
