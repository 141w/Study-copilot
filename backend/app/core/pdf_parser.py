...
class PDFParser:
    ...
    def _convert_sync(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"metadata": {}, "markdown": "", "success": False, "error": str(e)}
    ...
    async def parse(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            result = await loop.run_in_executor(_executor, self._convert_sync, file_path)
    ...
    def _split_markdown_pages(self, markdown_text: str, page_count: int) -> list[dict[str, Any]]:
        ...
        if "---" in markdown_text:
            ...
            for idx, part in enumerate(parts):
                part = part.strip()
                if part:
                    pages.append({"page": idx + 1, "text": part})
            if pages:
                return pages
        ...
        if page_count > 0:
            ...
            if len(paragraphs) <= page_count:
                ...
                for idx, para in enumerate(paragraphs):
                    pages.append({"page": idx + 1, "text": para})
            else:
                # 段落多，均分到各页
        ...
        # 最终兜底：整篇文档作为一页
        if not pages:
            pages.append({"page": 1, "text": markdown_text})
