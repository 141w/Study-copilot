...
class BM25VectorStore(BaseVectorStore):
    ...
    async def save(self, path: str) -> bool:
        ...
        with open(f"{path}.bm25", "w", encoding="utf-8") as f:
            json.dump(
                {"chunks": self.chunks, "document_ids": self.document_ids, "corpus": self._corpus},
                f,
                ensure_ascii=False,
            )
...
class HybridVectorStore(BaseVectorStore):
    ...
    def get_chunk_count(self) -> int:
        """获取块数量"""
        return max(self._faiss.get_chunk_count(), self._bm25.get_chunk_count())
...
class DocumentVectorStore:
    ...
    def __init__(
        ...
        if retrieval_type == self.RETRIEVAL_TYPE_BM25:
            ...
            self._store = FAISSVectorStore(dimension=self.dimension)
        else:  # hybrid
            self._store = HybridVectorStore(dimension=self.dimension)
