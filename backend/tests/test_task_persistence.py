...
async def test_pending_row_claimed_and_executed(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        task_worker, "AsyncSessionLocal", _make_sessionmaker(engine)
    )
    ...
    async with maker() as db:
        probe = (await db.execute(
            select(task_service.AsyncTask)
        )).scalars().all()
        print("PROBE rows:", [(r.id[:8], r.status) for r in probe],
...
async def test_worker_loop_polls_pending_rows_autonomously(tmp_path, monkeypatch):
    ...
    async with maker() as db:
        await task_service.create_task(
            db, uid, "unknown_type_for_test", {"src": "restart-before-enqueue"},
        )
    ...
    try:
        ...
        while loop.time() < deadline:
            async with maker() as db:
                row = (
                    await db.execute(
                        select(task_service.AsyncTask).where(
                            task_service.AsyncTask.user_id == uid
                        )
                    )
                ).scalar_one()
                status = row.status
...
async def test_recover_marks_only_running_orphans(db_session: AsyncSession):
    ...
    db_session.add(AsyncTask(
        ...
        user_id=uid, task_type="document_process", status="running", progress=0.5,
    ))
    db_session.add(AsyncTask(
        id="t-queued-pending-" + uuid.uuid4().hex[:6],
        user_id=uid, task_type="document_process", status="pending", progress=0.0,
    ))
    ...
    rows = (
        await db_session.execute(
            select(AsyncTask).where(AsyncTask.user_id == uid)
        )
    ).scalars().all()
